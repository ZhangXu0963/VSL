...
class PrecompDataset(data.Dataset):
    ...
    def __init__(self, data_path, data_split, vocab):
        ...
        self.captions = []
        # load TFIDF
        tfidf_path = self.loc+'CIDEr_%s.h5' % data_split
        self.tfidfs =  h5py.File(tfidf_path, 'r')
    ...
    def __getitem__(self, index): #index: caption的编号
        ...
        vocab = self.vocab
        sem_for_caption = self.tfidfs['TFIDF'][index] # self.caption's tfidf
        sems_for_image = self.tfidfs['TFIDF'][img_id*5: (img_id+1)*5] # captions' tfidf matched with image
        # -------- The main difference between python2.7 and python3.6 --------#
        # The suggestion from Hongguang Zhu(https://github.com/KevinLight831)
        # ---------------------------------------------------------------------#
