...
def evalrank(model_path, data_path=None, split='dev', fold5=False):
    ...
    if not fold5:
        ...
        # np.save('path', sims)
        sims = np.load('path/sims43_5K.npy')
        sims_other = np.load('path/sims48_epoch25_5K.npy')
        sims = (sims + sims_other) / 2

        # np.savetxt('path', sims.T, delimiter=',')
        ...
        results = []
        # sims_all = []
        sims_all = np.load('path/sims43_1K.npy')
        sims_all_other = np.load('path/sims48_epoch25_1K.npy')
        ...
        for i in range(5):
            ...
            sims = (sims_all[i] + sims_all_other[i]) / 2
...
def i2t(images, captions, caplens, sims, npts=None, return_ranks=False):
    ...
    ranks = np.zeros(npts)
    top1 = np.zeros(npts)
    # sims_gt = np.zeros([npts, 5]) #跟gt的sims
    # sims_top1 = np.zeros([npts, 5])
    # index_all = []
...
if __name__ == '__main__':
    evalrank(model_path="model_path/checkpoint_22.pth.tar",
             data_path='data_path', split="testall", fold5=False) #mscoco1k: fold5=True, else: fold5=False
